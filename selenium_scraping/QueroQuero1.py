...
#Targets  = load_pkl(name='Targets')
#Eans     = load_pkl(name='Eans')
#Strategy = load_pkl(name='Strategics')['strategyEAN']
#eansDif  = set(Eans) - set(Targets['targets']['EAN'])
#eansDif  = list(eansDif)
#urls    = target[f'Link {guideshop}'].astype({f'Link {guideshop}':'str'})#[t:210]
#eans     = Strategy#eansDif
...
time1            = time.time()
with open(out_path + f'QueroQuero2_{today}.csv', "w", newline="", encoding="utf-8-sig") as f:
         ...
         for ean in eans:
            # link = url
             try:
                ...
                try:
                    if soup.find('p', {'class','lh-copy vtex-rich-text-0-x-paragraph vtex-rich-text-0-x-paragraph--text'}).text=='Indisponível':
                        continue
                    else:
                        try:
